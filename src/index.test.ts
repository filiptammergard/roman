...
import { fromRoman, pattern, toRoman } from "./index.js"
...
it("converts known pairs in both directions", () => {
	for (const [arabic, roman] of KNOWN_PAIRS) {
		...
		expect(fromRoman(roman)).toEqual(arabic)
	}
})

it("roundtrips every value from 1 to 3999", () => {
	for (let n = 1; n < 4000; n++) {
		expect(fromRoman(toRoman(n))).toEqual(n)
	}
})
...
it("throws on invalid arabic input", () => {
	...
	expect(() => toRoman(0)).toThrow("0 cannot be represented in roman numerals")
